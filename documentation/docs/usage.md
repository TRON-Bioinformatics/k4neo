...
* **`sample-tables`**: Path to sample annotation table (see [k4neo-index-data]() for further information).
* **`database`**: Path to database file.
* **`tissue-map`**: Path to tissue mapping to translate public tissue nomenclature to k4neo tissue definition (see [k4neo-index-data]() for further information).
...
* **`transcriptome`**:  Path to reference transcriptome fasta file
* **`kmer`**: k-mer size for index construction
...
k4neo 1.0.1 uniqueness annotation
...
* **`fasta`**: Path to fasta file with query sequences, e.g. `query.fa` generated by `k4neo-annotator`.
* **`reference_indices`**: Path to reference index manifest.
...
k4neo 1.0.1 quantitative annotation
...
* `profile`: Path to yaml file with additional snakemake options. Can be used to customize snakemake execution, different executor plugins etc.
* `cpu`: Number of cpus for local execution or number of jobs if submitting to slurm.
* `normalize`: Normalize k-mer counts returned by index.
* `normalize-factor`: Normalization factor for k-mer counts. Default is 1e9.
