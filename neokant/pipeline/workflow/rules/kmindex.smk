...
rule ntcard:
    ...
    params:
        kmer_size ='',
        prefix = lambda wildcards, output: output.rstrip(".hist")
    shell:
        'ntcard '
        '--kmer={params.kmer_size} '
        '--pref={params.prefix} '
        '{input.fastq} && '
        'mv index/ntcard/{sample}_k*.hist index/ntcard/{sample}.hist'
...
rule gather_ntcard:
    ...
    output:
        ...
        kmer_all_sorted = 'index/ntcard/experiments.ntcard.txt'
    shell:
        '''
...
rule estimate_bf_size:
    ...
    params:
        bf_size_exe = 'simple_bf_size_estimate.py'
    output:
        bloom_filter_size = 'index/kmindex/bloom_filter_size.txt'
    shell:
        '''
        largest_experiment="$(grep -v '#' {input.kmer_all_experiments} | head -n 1 | cut -d ' ' -f 4)"
...
rule kmtricks:
    input:
        bf_size = rules.estimate_bf_size
        sample_sheet = config['indexing']['samples']
    output:
        kmtricks_index = dir('index/kmindex/kmtricks')
    threads: 16
    params:
...
rule kmindex:
    ...
    output:
        kmindex_index = dir('index/kmindex/global_index')
    shell:
        '''
        # Delete automatically generated output dir 
        test -d {output.kmindex_index} && rmdir {output.kmindex_index}
        kmindex -i {output.kmindex_index} -n samples -p {input.kmtricks_index}"
        '''