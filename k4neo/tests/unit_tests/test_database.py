...
def example_database(mocker):
    fake_data = [
        {
            "tissue_public": "subcutaneous adipose tissue",
            "tissue": "adipose tissue",
            "subtissue": "subcutaneous",
        },
        {
            "tissue_public": "mesenteric fat pad",
            "tissue": "adipose tissue",
            "subtissue": "mesenteric fat pad",
        },
...
def test_tissue_table_initialisation(mocker):
    ...
    assert inserted_elements.get("tissue", pd.Series()).unique().tolist() == [
        "adipose tissue",
        "adrenal gland",
    ]
    assert inserted_elements.get("subtissue", pd.Series()).unique().shape[0] == 3
    assert inserted_elements.get("subtissue", pd.Series()).unique().tolist() == [
        "subcutaneous",
        "mesenteric fat pad",
        "NA",
    ]
